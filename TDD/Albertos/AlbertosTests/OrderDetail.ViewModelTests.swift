...
struct OrderDetail_ViewModelTests {
  ...
  func test1() {
    ...
    let sut = OrderDetail.ViewModel(orderController: orderController,       onAlertDismiss: {}
    )
  ...
  func test6() {
    ...
    let viewModel = OrderDetail.ViewModel(
      orderController: orderController, onAlertDismiss: {},
  ...
  @Test("결제 완료 알림창이 닫힐때, 전달한 클로저가 잘 실행되는지 확인")
  func test9() async throws {
    var called = false
    let viewModel = OrderDetail.ViewModel(
      orderController: OrderController(), onAlertDismiss: { called = true },
      paymentProcessor: PaymentProcessingStub(
        returning: .failure(TestError(id: 123))
      )
    )
